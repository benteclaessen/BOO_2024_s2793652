...
## Raw counts
```{r}
countdata_raw = fread(file.path(input_dir, "20240603filtered_countdata_raw_processed_rat.csv"))
```
...
order <- c("RPPTEC_CSA_8hr_2", "RPPTEC_CSA_8hr_10", "RPPTEC_CSA_8hr_40", "RPPTEC_CSA_24hr_2", "RPPTEC_CSA_24hr_10", "RPPTEC_CSA_24hr_40", "RPPTEC_CSA_72hr_2", "RPPTEC_CSA_72hr_10", "RPPTEC_CSA_72hr_40")

# Assign status to every gene
red <- deseq_results %>%
...
# Generate scatter plot with significant and not significant genes per treatment condition
plot <- ggplot(data = red, aes(x = log2FoldChange, y = -log10(padj), color = class)) + 
  facet_wrap(~MEAN_ID_TREATMENT, scales = "fixed") +  
  geom_point(size = 0.5) +
  geom_text(data = filter(red, abs(log2FoldChange) > 20), aes(label = gene_symbol), vjust = -1, hjust = 1, size = 2, show.legend = FALSE) +  
  theme_bw()
...
# We print the output
  autoplot(object = pca_object, data = pca_data, colour = "TIMEPOINT", size="DOSE") + 
    theme_bw()
```
...
# Save output
```{r}
write_rds(deseq_results, file.path(output_dir, paste0(gsub( "-", "", Sys.Date()), "_DESeq2_results_unformatted_rat2.rds")))
write_delim(expression_long_TXG, file.path(output_dir, paste0(gsub( "-", "", Sys.Date()), "_expression_long_TXG_upload_rat2.txt")))
