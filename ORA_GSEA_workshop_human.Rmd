...
## DESeq2 output
```{r}
deseq_results = readRDS(file.path(output_dir, "20240603_DESeq2_results_unformatted_human2.rds"))
...
human_rgd$RAT_GENE_SYMBOL <- rat_ncbi$gene_symbol[match(human_rgd$RAT_GENE_NCBI_GENE_ID,rat_ncbi$entrez_id)]
...
# Voeg kolommen toe zoals nodig voor TXG-MAPr
...
expression_long_TXG <- expression_long_TXG %>%
  mutate(experiment = case_when(
    sample_id == "HPPTEC_CSA_8hr_2" ~ "1CYCLOSPORIN A",
    sample_id == "HPPTEC_CSA_8hr_10" ~ "2CYCLOSPORIN A",
    sample_id == "HPPTEC_CSA_8hr_40" ~ "3CYCLOSPORIN A",
    sample_id == "HPPTEC_CSA_24hr_2" ~ "4CYCLOSPORIN A",
    sample_id == "HPPTEC_CSA_24hr_10" ~ "5CYCLOSPORIN A",
    sample_id == "HPPTEC_CSA_24hr_40" ~ "6CYCLOSPORIN A",
    sample_id == "HPPTEC_CSA_72hr_2" ~ "7CYCLOSPORIN A",
    sample_id == "HPPTEC_CSA_72hr_10" ~ "8CYCLOSPORIN A",
    sample_id == "HPPTEC_CSA_72hr_40" ~ "9CYCLOSPORIN A",
    TRUE ~ experiment 
  ))
...
```
```{r}
write_delim(expression_long_TXG, file.path(output_dir, paste0(gsub( "-", "", Sys.Date()), "_renamed_expression_long_TXG_upload_human.txt")))
```
...
```
```{r}
write_rds(heatmap_long, file.path(output_dir, paste0(gsub( "-", "", Sys.Date()), "_heatmap_long_human.rds")))
```
...
ggplot(heatmap_long, aes(x = Var2, y = Var1, fill = Freq)) +
  ...
  labs(
    title = "Overlapping Pathways Of The Top 20 by Lowest Adjusted P-value per Condition (ORA)",
    x = "MEAN_ID_TREATMENT",
    y = "Source",
    fill = "Adjusted P-value"
  ) +
...
## Run fgsea
```{r}
deseq_results_test = deseq_results_filtered %>% separate(gene_symbol,
...
```{r}
write_rds(heatmap_long_GSEA, file.path(output_dir, paste0(gsub( "-", "", Sys.Date()), "_heatmap_long_GSEA_human.rds")))
```
...
# Lees de datasets in
combined_top_results_human <- readRDS(file.path(output_dir, "20240619_heatmap_long_GSEA_human.rds"))
...
p_combined_heatmap <- ggplot(filtered_results, aes(x = Var2, y = Var1, fill = Freq)) +
  ...
  scale_fill_viridis(option = "viridis", na.value = "#D3D3D3") +
  labs(
